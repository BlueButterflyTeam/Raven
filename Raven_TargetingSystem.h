...
private:
  ...
  //list of bots having owner as current target
  std::list<Raven_Bot*> m_pAggresors;
...
public:
  ...
  //add an aggressor to the Owner
  void		 AddAggresor(Raven_Bot* aggressor){ m_pAggresors.push_back(aggressor);}

  //remove an aggressor of the Owner
  void		 RemoveAggresor(Raven_Bot* aggressor){ if (isAggressor(aggressor)){ m_pAggresors.remove(aggressor); } }

  //return true if the bot is an aggressor of the Owner
  bool		 isAggressor(Raven_Bot* bot);

  //return true if Owner has any aggressor
  bool		 isAggressed(){ return !m_pAggresors.empty(); }

  void		 ClearAggressor(){ m_pAggresors.clear(); }
};
