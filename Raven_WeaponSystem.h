...
private:
  ...
  //last calculated precision
  double            m_dLastPrecission;
...
public:
  ...
  //--------------------------------------------------------------
  double GetPrecision(double DistToTarget)const;
  FuzzyModule* GetFuzzyModule(){ return m_pFuzzyModule; }

  void TemperAimByPrecision(Vector2D& AimingPos)const;
};
