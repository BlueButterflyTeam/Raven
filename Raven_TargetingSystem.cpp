...
void Raven_TargetingSystem::Update()
{
  if (m_pCurrentTarget != NULL){
    m_pCurrentTarget->GetTargetSys()->RemoveAggresor(m_pOwner);
  }
  ...
  if (m_pCurrentTarget != NULL && isTargetShootable()){
	m_pCurrentTarget->GetTargetSys()->AddAggresor(m_pOwner);
  }
}
...
bool Raven_TargetingSystem::isAggressor(Raven_Bot* bot)
{
	if (m_pAggresors.empty()){ return false; }

	std::list<Raven_Bot*>::const_iterator curBot = m_pAggresors.begin();
	for (curBot; curBot != m_pAggresors.end(); ++curBot)
	{
		if ((*curBot) == bot){ return true; }
	}
