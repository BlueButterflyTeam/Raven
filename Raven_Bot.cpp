...
//-------------------------- ctor ---------------------------------------------
Raven_Bot::Raven_Bot(Raven_Game* world,Vector2D pos, double skill):
...
//-------------------------------------------------------------------------------------
bool Raven_Bot::canStepLeftOf(Vector2D& PositionOfStep)const
{
	static const double StepDistance = BRadius() * 2;

	PositionOfStep -= (Facing().Perp() * StepDistance + Facing().Perp() * BRadius());

	return canWalkTo(PositionOfStep);
}

bool Raven_Bot::canStepRightOf(Vector2D& PositionOfStep)const
{
	static const double StepDistance = BRadius() * 2;
