                         ...
                         m_pGraveMarkers(NULL)
{
	...
	m_pRandomGenerator = new std::default_random_engine(seed);

	m_pPoissonDistribution = new std::poisson_distribution<int>;

	//init Poisson Distribution for bot creation
	SetPoissonDistributionMean(50.0);
	//---------------------------------
...
{ 
  while (NumBotsToAdd--)
  {
	...
	double skillLevel = GetPoissonDistributionSample();

    //create a bot. (its position is irrelevant at this point because it will
    //not be rendered until it is spawned)
    Raven_Bot* rb = new Raven_Bot(this, Vector2D(),skillLevel);
